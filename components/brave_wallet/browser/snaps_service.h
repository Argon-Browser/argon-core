...
// Minimal browser-side service that exposes snap load/invoke operations to the
// wallet UI and owns the snap execution-environment bridge.
// snap.request() is answered by a mocked SnapRequestHandlerImpl.
class SnapsService : public mojom::SnapsService {
 ...
 private:
  ...
  void OnLoadSnapForInvokeResult(InvokeSnapCallback callback,
