...
void SnapsService::LoadSnap(const std::string& snap_id,
  ...
  if (!bridge_controller_->IsBound()) {
    std::move(callback).Run(false, "Wallet page is not running");
    return;
  }
  bridge_controller_->LoadSnap(
      snap_id, base::BindOnce(&SnapsService::OnLoadSnapResult,
...
void SnapsService::InvokeSnap(const std::string& snap_id,
  ...
  // page; fail if none is running.
  if (!bridge_controller_->IsBound()) {
    std::move(callback).Run(std::nullopt, "Wallet page is not running");
    return;
  }
  OnBridgeReady(snap_id, method, std::move(*params),
                /*caller_origin=*/"brave-wallet", std::move(callback));
}

void SnapsService::OnBridgeReady(const std::string& snap_id,
                                 const std::string& method,
                                 base::Value params,
  ...
  bridge_controller_->LoadSnap(
      snap_id,
      base::BindOnce(&SnapsService::OnLoadSnapForInvokeResult,
                     weak_ptr_factory_.GetWeakPtr(), std::move(callback),
                     snap_id, method, std::move(params), caller_origin));
}

void SnapsService::OnLoadSnapForInvokeResult(
    InvokeSnapCallback callback,
